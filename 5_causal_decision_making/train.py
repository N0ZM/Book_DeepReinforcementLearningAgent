from env import Env
from models import AgentBrain, ReasonableMotivator, ActionMaker, RandomAgent
...
for episode in range(episodes):
    ...
    while not done:
        ...
        # 最適化確認用
        ns = reasonable_motivator(mood, state, q).detach()
        loss_fn = nn.MSELoss()
        t_ns = torch.tensor(
            next_state[np.newaxis, :], dtype=torch.float32
        )
        loss = loss_fn(t_ns, ns).detach().numpy()
        losses.append(loss)
        ...
        next_state_2 = reasonable_motivator(mood, state, 3).detach().numpy()[0]
