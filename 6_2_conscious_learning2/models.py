...
class ReasonableMotivator(nn.Module):
    ...
    mood, state, valueを受け取りnext_stateを返す
...
# 逆方向の因果推論モデル
class CauseDetector(nn.Module):
    """
    ある状態になったのは何が起こったからなのかを判定するモデル
    state, next_stateを受け取りmood_valueを返す

    Args:
        state (ndarray): state_t0
        next_state (tensor): state_t1
    Returns:
        mood_value (tensor): agentの気分(意図)と行動価値
    """
...
# 順方向の因果推論モデルその2
class ReasonableMotivator2(nn.Module):
    """
    ある状況にある時次に何が起こるのかを判定するモデル
    ある条件下で次に何をするべきかを決定するモデルとも言える
    mood_value, stateを受け取りnext_stateを返す

    Args:
        mood_value (ndarray): agentの気分(意図)と行動価値
        state (ndarray): state_t0
    Returns:
        next_state (tensor): state_t1
    """
    ...
    def forward(self, mood_value, state):
        x = np.array(mood_value)
        x = np.append(x, state)
        x = torch.tensor(x[np.newaxis, :], dtype=torch.float32)
    ...
    def update(self, mood_value, state, t):
        t = torch.tensor(t[np.newaxis, :], dtype=torch.float32)
        y = self(mood_value, state)
        loss_fn = nn.MSELoss()
        loss = loss_fn(y, t)
        self.optimizer.zero_grad()
        loss.backward()
