...
for episode in range(episodes):
    ...
    while not done:
        ...
        pred_next_state = reasonable_motivator2(
            mood_reward, state).detach().numpy()[0]
        guess = cause_detector(state, pred_next_state)
        t = guess.clone().detach()
        # t = torch.tensor([[0, 0, 0]], dtype=torch.float32)
        t[0][idx_mood] = torch.tensor(q, dtype=torch.float32)
        loss_fn = nn.MSELoss()
        loss = loss_fn(guess, t)
        reasonable_motivator2.optimizer.zero_grad()
        loss.backward()
        reasonable_motivator2.optimizer.step()
    ...
    # if episode % 10 == 0:
    #     print("episode :{}, total loss : {:.1f}".format(
    #         episode, total_loss))
