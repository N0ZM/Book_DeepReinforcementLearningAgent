...
class Env:
    ...
    def get_reward(self, mood, prev_dist_target, now_dist_target,
        ...
        if mood == 2:
            ...
            reward += (prev_dist_enemy - now_dist_enemy) * 0.1
            done = is_hit(self.enemy, self.agent)
            if done:
                reward = 100
            return reward, done
    ...
    def step_to_get_reward(self, action, mood):
        ...
        reward, done = self.get_reward(mood, 
                                       prev_dist_target, now_dist_target, 
                                       prev_dist_enemy, now_dist_enemy)

        # 変更されたagentとenemyの値を元に戻す
        self.agent = agent
        self.enemy = enemy
        return next_state, reward, done
